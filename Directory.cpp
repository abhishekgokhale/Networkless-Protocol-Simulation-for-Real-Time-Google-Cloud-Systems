...
// Creating the send function
Json::Value
Directory::send(std::string arg_dir_fhandle, std::string arg_name)
{
  int i;
  int flag = 0;
  Json::Value result;
  ...
  if (arg_dir_fhandle == this->fhandle)
  {
    for (i = 0; i < (this->dirent_vector_ptr)->size(); i++)
    {
      ...
      {
        result["fhandle"] = ((*(this->dirent_vector_ptr))[i])->fhandle;
        result["name"] = ((*(this->dirent_vector_ptr))[i])->name;
        flag = 1;
        result["LookUp"] = "Found";
      }
    }
    ...
    {
      result["status"] = "NFSERR_NOENT";
      result["LookUp"] = "404: File Not Found";
    }
    else
    {
      result["status"] = "NFS_Sending";
    }
  }
  else
  {
...
(std::string arg_dir_fhandle, std::string arg_name, std::string arg_sattr, std::string newFile_fhandle, std::string newFile_name)
{
  int i;
  Json::Value result;
  ...
  if (arg_dir_fhandle == this->fhandle)
  {
    ...
    std::cout << "Received new file to update" << std::endl;
    std::cout << "Updating fhandle and name" << std::endl << std::endl;

    std::cout << "Old fhandle for New File -> " << this->fhandle << std::endl;
    std::cout << "Old name for New File -> " << ((*(this->dirent_vector_ptr))[i])->name << std::endl << std::endl;

    this->fhandle = newFile_fhandle;
    ((*(this->dirent_vector_ptr))[i])->name = newFile_name;

    std::cout << "Updated fhandle for New File -> " << this->fhandle << std::endl;
    std::cout << "Updated name for New File -> " << ((*(this->dirent_vector_ptr))[i])->name << std::endl << std::endl;

    std::cout << "Deleting old file" << std::endl;
    sleep(3);
    std::cout << "Old file deleted" << std::endl << std::endl;

	  result["status"] = "NFS_OK";
	}
      else
	{
	  result["status"] = "NFSERR_FILE_NOT_FOUND";
	}
...
// Adding The Zip Funciton
Json::Value
Directory::Zip(std::string arg_dir_fhandle, std::string arg_name, std::string arg_filetype, std::string arg_filesize)
{
  ...
  if (arg_dir_fhandle == this->fhandle)
  {
    for (i = 0; i < (this->dirent_vector_ptr)->size(); i++)
    {
      if (((*(this->dirent_vector_ptr))[i])->name == arg_name)
      {
        ...
        flag = 1;
        result["LookUp"] = "Found";
        result["CurrentSize"] = arg_filesize;
        result["CurrentType"] = arg_filetype;
      }
    }
    if (arg_filetype == "txt")
    {
      result["ZippingAlgo"] = "huffmancoding";
    }
    ...
    if (arg_filetype == "jpg")
    {
      result["ZippingAlgo"] = "LZ77";
    }

    result["SizeAfterZip"] = "1Mb";
